...
namespace Framework {
    ...
    class Model extends Base {
    	...
    	/**
    	 * Store Old values of columns in this array
    	 * @var array
    	 */
    	protected $oldValues = [];
        ...
        protected function setOldValues($values) {
        	$this->oldValues = $values;
        }

        public function getOldValue($column) {
        	return $this->oldValues[$column] ?? null;
        }
        ...
        protected function beforePropertySetHook($property, $value) {
        	$column = substr($property, 1);
        	$columns = $this->getColumns();
        	if (isset($columns[$column])) {
        		$this->oldValues[$column] = $this->$property;
        	}
        }

        public static function getCacheKey($query = [], $fields = []) {
        	$table = get_called_class();
        	$str = sprintf("Model_%s_%s_%s", $table, ArrayMethods::arraySig($query), json_encode($fields));
        ...
        public function getColumns() {
            if (empty($_columns)) {
                ...
                $first = function($array, $key) {
                    if (is_array($array) && !empty($array[$key]) && is_array($array[$key]) && count($array[$key]) == 1) {
                        return $array[$key][0];
                    }
                    return null;
                };
