...
use Framework\{Registry, Utils, TimeZone};
...
class Db {
	...
	public static function connect() {
		...
		if (!$mongoDB) {
			...
			try {
				...
				$dbURL = sprintf("mongodb://%s:%s@%s/%s?replicaSet=%s&authSource=admin", $dbconf->username, $dbconf->password, $dbconf->url, $dbconf->dbname, $dbconf->replica);
	...
	public static function convertType($value, $type = 'id') {
		switch ($type) {
			case 'id':
				return static::mongoObjectId($value);

			case 'regex':
				return static::mongoRegex($value);
	...
	public static function isType($value, $type = '') {
		switch ($type) {
			case 'id':
				return is_object($value) && is_a($value, static::ID);

			case 'regex':
				return is_object($value) && is_a($value, static::REGEX);
			...
			case 'datetime':
			case 'time':
				return is_object($value) && is_a($value, static::DATE);
	...
	public static function dateQuery($start = null, $end = null, $opts = []) {
		$changed = false;
		if ($start && $end) {
		...
		if (!$changed) {
			$dq = TimeZone::dateRangeQuery(['start' => $start, 'end' => $end], $opts);
		}
	...
	public static function removeModelCache($model, $query, $fields = []) {
		...
		$dbCacheKey = static::getCacheKey($model, $query, $fields);
		Utils::removeCache($cacheKey);
		Utils::removeCache($dbCacheKey);
	}
	...
	public static function aggregate($model, $query, $groupBy, $extra = []) {
		...
		if ($groupByDate) {
			if (! is_array($group['_id'])) {
				$group['_id'] = [];
				$group['_id'][$groupBy[0]] = sprintf("$%s", $groupBy[0]);
			}
