...
class Utils {
	 ...
	 */
	public static function debugMode($var = null) {
		$ip = static::getClientIp();
		if (in_array($ip, ['14.139.251.107', '47.31.8.59', '202.9.120.171'])) {
			if (!is_null($var)) {
				var_dump($var);	
			}
			return true;
	 ...
	 * @return string
	 */
	public static function getClientIp($default = '') {
		$headers = \Framework\Request::getallheaders();
		$ip = '';
		if (isset($headers['Cf-Connecting-Ip'])) {
			...
			$ip = RequestMethods::server('REMOTE_ADDR') ?? RequestMethods::server('HTTP_CLIENT_IP') ?? $default;
		}
		return $ip;
	}
	...
	public static function media($name, $task = 'show', $opts = []) {
		...
		switch ($task) {
			...
			case 'display':
				return sprintf("%suploads/images/%s", CDN, $name);
	...
	public static function getInfo($ip) {
		...
		if ($curl->error) return ['country' => 'XX'];
		$jsonBody = $curl->response;

		return ['country' => (isset($jsonBody->country) ? $jsonBody->country : 'XX')];
	}
}
